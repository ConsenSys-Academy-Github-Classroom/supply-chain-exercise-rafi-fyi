...
contract SupplyChain {
  ...
  modifier forSale(uint sku) {
    require (items[sku].state == State.ForSale && items[sku].seller != address(0), "Item is not for sale");
    _;
  }
  ...
  constructor() {
    ...
    skuCount = 0; // this is not necessary
  }
  ...
  function buyItem(uint sku) public payable forSale(sku) paidEnough(items[sku].price) checkValue(sku) {
    ...
    Item storage currItem = items[sku];
    currItem.buyer = payable(msg.sender);
    currItem.state = State.Sold;
    emit LogSold(sku);
  }
